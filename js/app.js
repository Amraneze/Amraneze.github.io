;(function(window, QB, app, CONFIG, $, Backbone) {
    ...
    $(function() {
		...
		$(document).on('click', '.j-actions-measurements', function() {
			var $btn = $(this);
			isMeasurements = $btn.data('call') === 'measurements';
			if(_.isEmpty(app.callees)) {
				$('#error_no_calles').modal();
				/*document.getElementById('measurements-div').style.visibility = "hidden";					
				document.getElementById('measurements-div').style.display = "none";	
				document.getElementById('no-measurements').style.visibility = "visible";	
				document.getElementById('no-measurements').style.display = "block";	*/
				initMeasurementsView(true);
				return false;
            }
			if(isMeasurements) {
				getMeasurements();
